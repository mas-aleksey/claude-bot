...
Корни перечислены в `FILE_ROOTS` и больше нигде. Всё, что приходит от браузера,
проходит через `inside`: путь сверяется с корнями после `resolve()`.
"""
...
# Корни дерева файлов, целиком. `/projects` стоит одним корнем, а не каждым проектом по
# отдельности: список тогда не зависел от того, что примонтировано, новый проект
# появляется в дереве сам, и сам `/projects` открывается — раньше он отвечал «путь вне
# корней», хотя всё его содержимое было доступно. Цена — один клик, чтобы войти в
# проект, и тот разовый: последний открытый каталог панель помнит.
#
# Остальные три — конфиг claude, источник скиллов и состояние бота: правят и смотрят их
# чаще всего, а лежат они вне /projects. Все пути есть у любого инстанса, поэтому дефолт,
# а не строка в каждом .env. Список через запятую и из окружения, как PROJECTS_DIR и
# PROJECTS_DIR: инстанс с иным набором монтирований переопределяет его у себя. Кнопка
# «добавить корень» из панели означала бы «добавить /», после чего список теряет смысл.
FILE_ROOTS = [Path(x.strip()) for x in
              os.environ.get("FILE_ROOTS", "/projects,/root/.claude,/opt/skills,/data")
              .split(",") if x.strip()]
...
def roots() -> list[Path]:
    """Существующие корни из FILE_ROOTS. Несуществующие пропускаем — инстансы монтируют
    разное, и лишний путь в .env не должен оставлять панель без списка."""
    return [r for r in FILE_ROOTS if r.is_dir()]
