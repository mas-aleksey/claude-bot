...
def count(cwd: str) -> int:
    """Сколько сессий у проекта. Только перечисление каталога: заголовки не читаем, а
    ради числа в свёрнутой строке дерева читать их было бы дорого — в `recent` на каждую
    сессию приходится чтение транскрипта и запрос в sqlite."""
    return sum(1 for _ in (TRANSCRIPTS / slug(cwd)).glob("*.jsonl"))
