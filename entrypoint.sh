...
# общие для всех инстансов, свои для типа инстанса (sandbox/assistant) и свои для
# проекта. Маунт папки поверх папки скрыл бы все, кроме последней, поэтому источники
# монтируются в /opt/skills/* (rw — claude правит скиллы сам), а сюда линкуются по
# одному скиллу.
#
# Порядок — алфавитный по имени каталога, и каждый следующий перебивает одноимённый
# скилл предыдущего. Поэтому точки монтирования пронумерованы (10-shared, 20-sandbox,
# 30-project): слабейший источник первым, проектный последним. Без цифр порядок дало
# бы само слово, и `shared` оказался бы сильнее всех — ровно наоборот.
#
